...
export function StudentRecordingsPage() {
  ...
  return (
    <StudentShell activeLabel="Recordings" breadcrumb="Learning / Recordings" summary={summary} isAdultStudent={isAdultStudent}>
...
async function loadRecordings() {
  ...
  const result = await callable({ limit: 200, activeRole: "student" });
...
async function toggleRecording(
  ...
  try {
    ...
    const result = await callable({ recordingId: recording.recordingId, activeRole: "student" });
    const raw = result.data;
...
function initialsFromName(name: string) {
  ...
  return (parts[0]?.[0] ?? "S").concat(parts[1]?.[0] ?? parts[0]?.[1] ?? "T").toUpperCase();
}
