    ...
    <TeacherShell activeLabel="Settings" breadcrumb="Account / Settings" summary={summary}>
      <div className="min-h-full bg-[#F8FAFC]">
        <header className="flex items-center gap-3 border-b border-[#E2E8F0] bg-white px-4 py-3 lg:hidden">
          ...
          <p className="font-extrabold">{tr("Settings")}</p>
        </header>
        <div className="mx-auto max-w-3xl space-y-5 p-4 sm:p-6 lg:p-8">
          <div><h1 className="text-2xl font-extrabold text-[#111827]">{tr("Settings")}</h1><p className="mt-1 text-sm text-[#64748B]">{tr("Manage your account and app preferences.")}</p></div>
          <SettingsCard title={tr("PROFILE")}><SettingsLink href="/teacher/profile/" icon={CircleUserRound} title={tr("View Profile")} subtitle="Profile photo and teaching information" /></SettingsCard>
          <SettingsCard title={tr("ACCOUNT SECURITY")}><SettingsButton icon={KeyRound} title={tr("Change Password")} subtitle="Update your account password" onClick={() => setPasswordOpen(true)} /></SettingsCard>
          <SettingsCard title={tr("APP SETTINGS")}>
            <SettingsButton icon={Bell} title={tr("Notifications")} subtitle="Shift, task, chat, and prayer reminders" onClick={() => setNotificationsOpen(true)} />
            <label className="flex min-h-16 items-center gap-3 border-t border-[#E2E8F0] px-4"><IconBox icon={Globe2} /><span className="min-w-0 flex-1"><strong className="block text-sm text-[#334155]">{tr("Language")}</strong><span className="text-xs text-[#64748B]">{tr("Choose the display language")}</span></span><select aria-label={tr("Language")} value={language} onChange={(event) => { const next = event.target.value === "fr" ? "fr" : "en"; setLanguage(next); void setLocale(auth.currentUser?.uid ?? null, next); }} className="h-10 rounded-xl border border-[#CBD5E1] bg-white px-3 text-sm font-bold"><option value="en">{tr("English")}</option><option value="fr">{tr("Français")}</option></select></label>
            <div className="border-t border-[#E2E8F0]"><SettingsButton icon={dark ? Moon : Sun} title={tr("Dark Mode")} subtitle={dark ? "Enabled" : "Disabled"} onClick={() => setTheme(!dark)} trailing={<Switch checked={dark} />} /></div>
          </SettingsCard>
          <SettingsCard title={tr("SUPPORT")}>
            <a href="mailto:support@alluwaleducationhub.org" className="block"><SettingsRow icon={Mail} title={tr("Help & Support")} subtitle="support@alluwaleducationhub.org" /></a>
...
function NotificationDialog({ preferences, onSaved, onClose }: { preferences: NotificationPrefs; onSaved: (value: NotificationPrefs) => void; onClose: () => void }) {
  ...
  return <section className="fixed inset-0 z-[90] grid items-end bg-black/45 sm:place-items-center" role="dialog" aria-modal="true" aria-label={tr("Notification preferences")}><div className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-[#F8FAFC] p-5 shadow-2xl sm:max-w-2xl sm:rounded-3xl sm:p-6"><header className="flex items-start gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-blue-100 text-[#0386FF]"><Bell size={21} /></span><div className="min-w-0 flex-1"><h2 className="text-xl font-extrabold text-[#111827]">{tr("Notification Preferences")}</h2><p className="text-sm text-[#64748B]">{tr("Choose which reminders you receive.")}</p></div><button type="button" onClick={onClose} aria-label={tr("Close notification preferences")} className="grid h-10 w-10 place-items-center rounded-xl"><X size={20} /></button></header><div className="mt-5 space-y-4"><PreferenceCard icon={Clock3} title={tr("SHIFT REMINDERS")} description="Get notified before your shift starts" checked={draft.shiftEnabled} onToggle={(value) => set("shiftEnabled", value)}>{draft.shiftEnabled ? <OptionChips label="Notify me before shift" options={[10, 15, 20, 30]} value={draft.shiftMinutes} suffix="min" onChange={(value) => set("shiftMinutes", value)} /> : null}</PreferenceCard><PreferenceCard icon={ListChecks} title={tr("TASK REMINDERS")} description="Get notified before a task due date" checked={draft.taskEnabled} onToggle={(value) => set("taskEnabled", value)}>{draft.taskEnabled ? <OptionChips label="Notify me before due date" options={[1, 2, 3, 5, 7]} value={draft.taskDays} suffix="day" onChange={(value) => set("taskDays", value)} /> : null}</PreferenceCard><PreferenceCard icon={MessageSquare} title={tr("CHAT MESSAGES")} description="Get notified when you receive messages" checked={draft.chatEnabled} onToggle={(value) => set("chatEnabled", value)} /><PreferenceCard icon={Bell} title={tr("PRAYER TIMES (ADHAN)")} description="Adhan reminders at all five daily prayer times" checked={draft.prayerEnabled} onToggle={(value) => set("prayerEnabled", value)}>{draft.prayerEnabled ? <p className="mt-3 border-t border-[#E2E8F0] pt-3 text-xs leading-5 text-[#64748B]">{tr("Prayer reminders use your device and browser location settings.")}</p> : null}</PreferenceCard></div>{message ? <p role="status" className={`mt-4 rounded-xl p-3 text-sm font-semibold ${message.includes("saved") ? "bg-emerald-50 text-emerald-800" : "bg-red-50 text-red-800"}`}>{message}</p> : null}<div className="mt-5 flex justify-end gap-3"><button type="button" disabled={busy} onClick={onClose} className="min-h-11 px-4 font-bold text-[#64748B]">{tr("Close")}</button><button type="button" disabled={busy} onClick={() => void save()} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#0386FF] px-5 font-bold text-white disabled:opacity-60"><Save size={18} />{busy ? tr("Saving…") : tr("Save Preferences")}</button></div></div></section>;
}
...
function PasswordDialog({ user, onClose }: { user: User; onClose: () => void }) {
  ...
  return <section className="fixed inset-0 z-[90] grid items-end bg-black/45 sm:place-items-center" role="dialog" aria-modal="true" aria-label={tr("Change password")}><form onSubmit={(event) => { event.preventDefault(); void save(); }} className="w-full rounded-t-3xl bg-white p-6 shadow-2xl sm:max-w-md sm:rounded-3xl"><h2 className="text-xl font-extrabold">{tr("Change Password")}</h2><div className="mt-5 space-y-4">{[["Current password", current, setCurrent], ["New password", next, setNext], ["Confirm new password", confirm, setConfirm]].map(([label, value, setter]) => <label key={label as string} className="block text-sm font-bold text-[#374151]">{label as string}<input type="password" required value={value as string} onChange={(event) => (setter as (value: string) => void)(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-[#CBD5E1] px-3 font-normal" /></label>)}</div>{message ? <p role="status" className={`mt-4 text-sm font-semibold ${message.includes("successfully") ? "text-emerald-700" : "text-red-700"}`}>{message}</p> : null}<div className="mt-6 flex justify-end gap-3"><button type="button" disabled={busy} onClick={onClose} className="min-h-11 px-4 font-bold text-[#64748B]">{tr("Cancel")}</button><button type="submit" disabled={busy} className="min-h-11 rounded-xl bg-[#0386FF] px-5 font-bold text-white disabled:opacity-60">{busy ? tr("Saving…") : tr("Change Password")}</button></div></form></section>;
}
