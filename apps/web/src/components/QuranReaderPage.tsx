...
export type AccessState = "checking" | "signedOut" | "allowed" | "denied";
...
/**
 * The Qur'an reader itself, with no opinion about who is reading it.
 *
 * Students and teachers get the same reader in the Flutter app (one
 * `QuranReaderScreen`, memorization keyed by uid), so the web keeps one
 * implementation too: the role page resolves the session and supplies its own
 * shell and access prompt, and everything below is shared. Duplicating this
 * file per role would guarantee the two drift apart.
 */
export function QuranReaderPage({ session, accessPrompt, renderShell }: {
  session: { access: AccessState; uid: string | null };
  accessPrompt: ReactNode;
  renderShell: (children: ReactNode) => ReactNode;
}) {
  const t = useT();
  const { access, uid } = session;
  ...
  if (access !== "allowed") return <>{accessPrompt}</>;
