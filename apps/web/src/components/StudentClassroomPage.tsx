...
export function StudentClassroomPage() {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, async (user) => {
      ...
      setAccess("checking");
      try {
        const allowed = (await resolveStudentSession(user)).isStudent;
        if (!mounted) return;
...
function buildZoomMeetingUrl(data: ZoomJoinResult) {
  const params = new URLSearchParams({
    ...
    // Leaving the meeting returns to the student Classes page, not the Flutter
    // app. This is the whole reason the join runs here rather than handing off.
    returnUrl: `${window.location.origin}/student/classes/`,
    embedded: "0",
