    ...
    <TeacherShell activeLabel="My Report" breadcrumb="Reports / My Report" summary={summary}>
      <div className="min-h-full bg-[#F8FAFC]">
        <div className="flex items-center gap-3 border-b border-[#E2E8F0] bg-white px-4 py-3 lg:hidden">
          <button type="button" aria-label={tr("Open teacher menu")} onClick={openTeacherMobileMenu} className="grid h-11 w-11 place-items-center rounded-xl"><Menu size={22} /></button>
          <div><p className="text-sm text-[#64748B]">{tr("Reports")}</p><p className="font-bold text-[#111827]">{tr("My Performance Audit")}</p></div>
        </div>
        <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div><h1 className="text-2xl font-extrabold text-[#111827]">{tr("My Performance Audit")}</h1><p className="mt-1 text-sm text-[#64748B]">{tr("Review your monthly classes, punctuality, forms, and performance.")}</p></div>
            <div className="flex items-center gap-2">
              {audits.length ? <select aria-label={tr("Report month")} value={month} onChange={(event) => setMonth(event.target.value)} className="h-11 rounded-xl border border-[#CBD5E1] bg-white px-3 font-semibold text-[#334155]">{audits.map((item) => { const value = stringValue(item.yearMonth); return <option key={item.id} value={value}>{monthLabel(value)}</option>; })}</select> : null}
              <button type="button" aria-label={tr("Refresh report")} disabled={loading || !user} onClick={() => user && void loadAudits(user.uid, setAudits, setMonth, setError, setLoading, month)} className="grid h-11 w-11 place-items-center rounded-xl border border-[#CBD5E1] bg-white text-[#0386FF] disabled:opacity-50"><RefreshCw size={18} className={loading ? "animate-spin" : ""} /></button>
            </div>
          </div>
...
function TeacherReportActions({ audit, busy, message, onAcknowledge, onDispute }: { audit: AuditData; busy: boolean; message: string; onAcknowledge: () => Promise<void>; onDispute: (field: string, reason: string, suggestedValue: string) => Promise<boolean> }) {
  ...
  return <div className="mt-5 space-y-5">
    <section className="rounded-2xl border border-[#E2E8F0] bg-white p-5"><h2 className="text-lg font-extrabold text-[#111827]">{tr("Report Actions")}</h2>{acknowledgedAt ? <p className="mt-3 flex items-center gap-2 text-sm font-semibold text-emerald-700"><CheckCircle2 size={18} />{tr("Acknowledged")} {new Intl.DateTimeFormat(dateLocale(), { dateStyle: "medium", timeStyle: "short" }).format(acknowledgedAt)}</p> : <button type="button" disabled={busy} onClick={() => void onAcknowledge()} className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#0386FF] px-4 font-bold text-white disabled:opacity-50"><CheckCircle2 size={18} />{tr("Acknowledge that I have read this report")}</button>}<Link href={coachId ? `/teacher/chat/?contact=${encodeURIComponent(coachId)}` : "/teacher/chat/"} className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#CBD5E1] font-bold text-[#334155]"><MessageSquare size={18} />{tr("Open discussion")}</Link>{message ? <p role="status" className={`mt-3 text-sm font-semibold ${/successfully/i.test(message) ? "text-emerald-700" : "text-red-700"}`}>{message}</p> : null}</section>
    <section className="rounded-2xl border border-[#E2E8F0] bg-white p-5"><h2 className="text-lg font-extrabold text-[#111827]">{tr("Request a Correction")}</h2><p className="mt-1 text-sm text-[#64748B]">{tr("If something is inaccurate, tell the review team what should be corrected.")}</p>
      {Object.keys(dispute).length ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><div className="flex items-center justify-between gap-3"><strong>{tr("Existing correction request")}</strong><span className="rounded-full bg-amber-200 px-2 py-1 text-xs font-bold uppercase">{stringValue(dispute.status) || "pending"}</span></div><p className="mt-2"><strong>{tr("Field:")}</strong> {disputeFieldLabel(stringValue(dispute.field))}</p><p><strong>{tr("Reason:")}</strong> {stringValue(dispute.reason)}</p>{stringValue(dispute.adminResponse) ? <p className="mt-2"><strong>{tr("Admin response:")}</strong> {stringValue(dispute.adminResponse)}</p> : null}</div> : null}
      {canDispute ? <form className="mt-4 space-y-3" onSubmit={async (event) => { event.preventDefault(); if (!field) { setValidation("Select the field you want corrected."); return; } if (reason.trim().length < 20) { setValidation("Please provide at least 20 characters explaining the correction."); return; } setValidation(""); if (await onDispute(field, reason.trim(), suggestedValue.trim())) { setReason(""); setSuggestedValue(""); } }}><label className="block text-sm font-bold text-[#334155]">{tr("Field to correct")}<select aria-label={tr("Field to correct")} value={field} onChange={(event) => setField(event.target.value)} className="mt-1 h-11 w-full rounded-xl border border-[#CBD5E1] bg-white px-3 font-normal"><option value="">{tr("Select a field")}</option>{disputeFields.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="block text-sm font-bold text-[#334155]">{tr("Reason")}<textarea aria-label={tr("Correction reason")} value={reason} onChange={(event) => setReason(event.target.value)} rows={4} className="mt-1 w-full rounded-xl border border-[#CBD5E1] p-3 font-normal" placeholder={tr("Explain what is incorrect and why")} /></label><label className="block text-sm font-bold text-[#334155]">{tr("Suggested value (optional)")}<input aria-label={tr("Suggested value")} value={suggestedValue} onChange={(event) => setSuggestedValue(event.target.value)} className="mt-1 h-11 w-full rounded-xl border border-[#CBD5E1] px-3 font-normal" /></label>{validation ? <p role="alert" className="text-sm font-semibold text-red-700">{validation}</p> : null}<button type="submit" disabled={busy} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#0386FF] px-4 font-bold text-white disabled:opacity-50"><Send size={18} />{busy ? tr("Sending…") : tr("Send correction request")}</button></form> : null}
    </section>
  </div>;
}
...
function Overview({ audit, onExport }: { audit: AuditData; onExport: () => void }) {
  ...
  return <div className="space-y-5">
    ...
    <section><h2 className="mb-3 text-lg font-extrabold text-[#111827]">{tr("Score Breakdown")}</h2><div className="grid gap-3 sm:grid-cols-3"><RateCard label="Completion" value={numberValue(audit.completionRate)} weight="40%" /><RateCard label="Punctuality" value={numberValue(audit.punctualityRate)} weight="35%" /><RateCard label="Form Compliance" value={numberValue(audit.formComplianceRate)} weight="25%" /></div></section>
    <section><h2 className="mb-3 text-lg font-extrabold text-[#111827]">{tr("Quick Stats")}</h2><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{stats.map(([label, value]) => <div key={label} className="rounded-2xl border border-[#E2E8F0] bg-white p-4"><p className="text-sm text-[#64748B]">{label}</p><p className="mt-1 text-xl font-extrabold text-[#111827]">{value}</p></div>)}</div></section>
    {Object.keys(payment).length ? <section><h2 className="mb-3 text-lg font-extrabold text-[#111827]">{tr("Payment Summary")}</h2><div className="grid gap-3 rounded-2xl border border-[#E2E8F0] bg-white p-5 sm:grid-cols-4"><PaymentStat label="Gross" value={numberValue(payment.totalGrossPayment)} /><PaymentStat label="Penalties" value={-Math.abs(numberValue(payment.totalPenalties))} /><PaymentStat label="Bonuses" value={numberValue(payment.totalBonuses)} /><PaymentStat label="Final Amount" value={numberValue(payment.totalNetPayment)} emphasized /></div></section> : null}
    {arrayValue(audit.issues).length ? <section><h2 className="mb-3 text-lg font-extrabold text-[#111827]">{tr("Issues to Address")}</h2><div className="space-y-2">{arrayValue(audit.issues).map((raw, index) => { const item = objectValue(raw); return <div key={index} className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">{stringValue(item.description) || "Performance issue"}</div>; })}</div></section> : null}
    <button type="button" onClick={onExport} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#0E72ED] px-5 font-bold text-white"><Download size={19} />{tr("Download My Teaching Data (CSV)")}</button>
  </div>;
}
...
function DetailTable({ tab, audit }: { tab: Exclude<Tab, "Overview">; audit: AuditData }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-[#E2E8F0] bg-white"><div className="border-b border-[#E2E8F0] px-5 py-4"><h2 className="font-extrabold text-[#111827]">{tab}</h2></div>{rows.length ? <div className="divide-y divide-[#E2E8F0]">{rows.map((raw, index) => { const row = objectValue(raw); const title = stringValue(row.title ?? row.shift_name ?? row.formTitle ?? row.form_title ?? row.subject) || `${tab.slice(0, -1)} ${index + 1}`; const status = stringValue(row.status ?? row.clock_status) || "Recorded"; const date = dateLabel(row.shift_start ?? row.created_at ?? row.submittedAt ?? row.clock_in_time); return <div key={index} className="flex flex-wrap items-center justify-between gap-3 px-5 py-4"><div><p className="font-bold text-[#334155]">{title}</p><p className="text-sm text-[#64748B]">{date || "Date unavailable"}</p></div><span className="rounded-full bg-[#F1F5F9] px-3 py-1 text-xs font-bold capitalize text-[#475569]">{status}</span></div>; })}</div> : <p className="p-8 text-center text-[#64748B]">{tr("No")} {tab.toLowerCase()} {tr("recorded for this month.")}</p>}</section>;
}

function EmptyReport({ month }: { month: string }) { return <div className="grid min-h-80 place-items-center rounded-3xl border border-dashed border-[#CBD5E1] bg-white p-8 text-center"><div><BarChart3 className="mx-auto text-[#CBD5E1]" size={60} /><h2 className="mt-4 text-lg font-extrabold text-[#334155]">{tr("No audit data for")} {monthLabel(month)}</h2><p className="mt-2 text-sm text-[#64748B]">{tr("Your performance data will appear here when a monthly report is available.")}</p></div></div>; }
...
function monthLabel(value: string) { const match = /^(\d{4})-(\d{2})$/.exec(value); return match ? new Intl.DateTimeFormat(dateLocale(), { month: "long", year: "numeric" }).format(new Date(Number(match[1]), Number(match[2]) - 1, 1)) : value || "Selected month"; }
...
function dateLabel(value: unknown) { const raw = dateValue(value); return raw ? new Intl.DateTimeFormat(dateLocale(), { dateStyle: "medium", timeStyle: "short" }).format(raw) : ""; }
