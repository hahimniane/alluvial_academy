...
export function TeacherProfilePage() {
  ...
  return <TeacherShell activeLabel="Profile" breadcrumb="Account / Profile" summary={summary}>
    <div className="min-h-full bg-[#F8FAFC]">
      <MobileHeader title={tr("My Profile")} />
      <div className="mx-auto max-w-5xl p-4 sm:p-6 lg:p-8">
        <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-[#0E72ED] to-[#1E3A5F] p-6 text-white shadow-lg sm:p-8">
          <div className="flex flex-col items-center gap-5 sm:flex-row">
            <label className="group relative grid h-28 w-28 shrink-0 cursor-pointer place-items-center overflow-hidden rounded-full border-4 border-white bg-white/15" aria-label={tr("Change profile photo")}>
              {text(record.profile_picture_url) ? <img src={text(record.profile_picture_url)} alt={tr("Teacher profile")} className="h-full w-full object-cover" /> : <span className="text-3xl font-black">{summary.initials}</span>}
            ...
            <div className="min-w-0 flex-1 text-center sm:text-left"><h1 className="text-2xl font-black sm:text-3xl">{displayName}</h1><p className="mt-1 text-white/80">{text(profile.professional_title) || "Teacher"}</p><span className="mt-3 inline-flex rounded-full bg-white/20 px-3 py-1 text-xs font-black tracking-wider">{tr("TEACHER")}</span></div>
            <div className="flex flex-wrap justify-center gap-2"><button type="button" onClick={() => { setEditing(true); setError(""); setNotice(""); }} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-4 font-bold text-[#0E72ED]"><Edit3 size={18} />{tr("Edit Profile")}</button>{text(record.profile_picture_url) ? <button type="button" disabled={busy} onClick={() => void removePhoto()} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/60 px-4 font-bold text-white disabled:opacity-60"><Trash2 size={18} />{tr("Remove Photo")}</button> : null}</div>
          </div>
        </section>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          <Card title={tr("Contact & account")}><Info icon={Mail} label="Email" value={text(record["e-mail"]) || text(record.email) || user?.email || "Not set"} /><Info icon={Phone} label="Phone" value={text(record.phone_number) || text(record.phone) || "Not set"} /><Info icon={Clock3} label="Timezone" value={text(record.timezone) || "Not set"} /></Card>
          <Card title={tr("About")}><About label="Title" value={text(profile.professional_title)} /><About label="Bio" value={text(profile.biography)} /><About label="Experience" value={text(profile.years_of_experience)} /><About label="Specialties" value={text(profile.specialties)} /><About label="Education & certifications" value={text(profile.education_certifications)} /></Card>
        </div>
      </div>
...
function MobileHeader({ title }: { title: string }) { return <header className="flex items-center gap-3 border-b border-[#E2E8F0] bg-white px-4 py-3 lg:hidden"><button type="button" aria-label={tr("Open teacher menu")} onClick={openTeacherMobileMenu} className="grid h-11 w-11 place-items-center rounded-xl"><Menu size={22} /></button><p className="font-extrabold text-[#111827]">{title}</p></header>; }
...
function ProfileDialog({ draft, busy, error, onChange, onClose, onSave }: { draft: ProfileDraft; busy: boolean; error: string; onChange: (key: keyof ProfileDraft, value: string) => void; onClose: () => void; onSave: () => void }) {
  ...
  return <section className="fixed inset-0 z-[90] grid items-end bg-black/45 sm:place-items-center" role="dialog" aria-modal="true" aria-label={tr("Edit teacher profile")}><form onSubmit={(event) => { event.preventDefault(); onSave(); }} className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:max-w-xl sm:rounded-3xl sm:p-6"><header className="mb-5 flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-emerald-100 text-emerald-700"><UserRound size={21} /></span><div className="min-w-0 flex-1"><h2 className="text-xl font-extrabold text-[#111827]">{tr("Complete Profile")}</h2><p className="text-sm text-[#64748B]">{tr("Keep your teaching information accurate.")}</p></div><button type="button" onClick={onClose} aria-label={tr("Close profile editor")} className="grid h-10 w-10 place-items-center rounded-xl"><X size={20} /></button></header><p className="mb-5 rounded-xl bg-blue-50 p-3 text-sm text-blue-900">This profile is private to the app. Public website profiles are managed separately by an administrator.</p><div className="space-y-4">{fields.map(([key, label, rows]) => <label key={key} className="block text-sm font-bold text-[#374151]">{label}{rows > 1 ? <textarea value={draft[key]} rows={rows} onChange={(event) => onChange(key, event.target.value)} className="mt-2 w-full rounded-xl border border-[#CBD5E1] p-3 font-normal outline-none focus:border-emerald-500" /> : <input value={draft[key]} required={key === "full_name"} onChange={(event) => onChange(key, event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-[#CBD5E1] px-3 font-normal outline-none focus:border-emerald-500" />}</label>)}</div>{error ? <p role="alert" className="mt-4 text-sm font-semibold text-red-700">{error}</p> : null}<div className="mt-6 flex justify-end gap-3"><button type="button" disabled={busy} onClick={onClose} className="min-h-11 rounded-xl px-4 font-bold text-[#64748B]">{tr("Cancel")}</button><button type="submit" disabled={busy} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-emerald-600 px-5 font-bold text-white disabled:opacity-60"><Save size={18} />{busy ? tr("Saving…") : tr("Save Profile")}</button></div></form></section>;
}
