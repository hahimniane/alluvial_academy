...
async function getAllSchedules() {
  ...
  console.log(`Found ${teacherTemplates.size} teachers with active templates\n`);
  ...
  for (const [teacherId, templates] of teacherTemplates) {
    ...
    const teacherTz = teacherInfo.timezone;
    ...
    for (const template of templates) {
      ...
      for (const dayNum of selectedWeekdays) {
        ...
        // Convert time to teacher's timezone
        ...
        const studentNames = (template.student_ids || []).map(sid => {
          const info = userMap.get(sid);
          return info ? info.name : sid;
        }).join(', ');
    ...
    // Build output
    let output = `Schedule for ${teacherInfo.name} (${teacherTz.replace('_', ' ')})\n\n`;
    
    for (const day of DAY_ORDER) {
      if (scheduleByDay.has(day)) {
        output += `${day.toUpperCase()}:\n`;
        for (const cls of scheduleByDay.get(day)) {
          output += `• ${cls.startTime} - ${cls.endTime} — ${cls.students}\n`;
        }
        output += '\n';
      }
    }
