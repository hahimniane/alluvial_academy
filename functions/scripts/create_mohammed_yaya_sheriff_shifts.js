...
// Schedules to create
const SCHEDULES = [
  // Saidou Barry - IC / Islamic (individual)
  {
    studentCodes: ['saidou.barry'],
    classType: 'IC',
    subject: 'islamic',
    subjectDisplay: 'Islamic',
    sessions: [
      { day: 3, startTime: '18:00', endTime: '19:00', durationMinutes: 60 }, // Wed 6-7 PM
      { day: 5, startTime: '19:00', endTime: '20:00', durationMinutes: 60 }, // Fri 7-8 PM
    ],
  },
  // FGC Group: Mamady Kaba & Djenab Kaba
  {
    studentCodes: ['mamady.kaba', 'djenabou.kaba'],
    classType: 'FGC',
    subject: 'islamic',
    subjectDisplay: 'Islamic',
    sessions: [
      { day: 6, startTime: '10:00', endTime: '12:00', durationMinutes: 120 }, // Sat 10 AM - 12 PM
      { day: 7, startTime: '10:00', endTime: '12:00', durationMinutes: 120 }, // Sun 10 AM - 12 PM
    ],
  },
  // Dalanda Diallo - IC / Islamic (individual)
  {
    studentCodes: ['dalanda.diallo'],
    classType: 'IC',
    subject: 'islamic',
    subjectDisplay: 'Islamic',
    sessions: [
      { day: 5, startTime: '18:00', endTime: '19:00', durationMinutes: 60 }, // Fri 6-7 PM
  ...
  // FGC Group: Alfa Sam & Rogui Sam
  {
    studentCodes: ['alfa.sam', 'rogui.sam'],
    classType: 'FGC',
    subject: 'islamic',
    subjectDisplay: 'Islamic',
    sessions: [
...
async function createShifts() {
  ...
  // 1. Find Mohammed Yaya Sheriff
  console.log('1. Finding teacher: Mohammed Yaya Sheriff...\n');
  ...
  for (const doc of teachersSnap.docs) {
    ...
    if ((fullName.includes('mohammed') || fullName.includes('muhammed')) && (fullName.includes('yaya') || fullName.includes('yayal')) && fullName.includes('sheriff')) {
      teacher = {
        uid: doc.id,
  ...
  if (!teacher) {
    ...
    for (const doc of teachersSnap.docs) {
      const data = doc.data();
      const fullName = `${data.first_name || ''} ${data.last_name || ''}`.trim().toLowerCase();
      if (fullName.includes('mohammed') || fullName.includes('yaya') || fullName.includes('sheriff')) {
