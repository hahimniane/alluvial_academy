...
const {
  ...
  buildBreakoutRoom,
} = require('../services/zoom/shift_zoom');
const { getZoomConfig } = require('../services/zoom/config');
const { findAvailableHost } = require('../services/zoom/hosts');
const { deleteMeeting: deleteZoomMeeting, updateMeeting: updateZoomMeeting } = require('../services/zoom/client');
...
const scheduleShiftLifecycle = onCall(async (request) => {
  ...
  try {
    ...
    // Block shift creation when all Zoom hosts are busy (teaching shifts only).
    // Backward compatibility: if Zoom isn't configured, or no hosts are configured, do not block.
    const shiftCategory = shiftData.shift_category || 'teaching';
    const isTeachingShift = shiftCategory === 'teaching' && shiftData.status !== 'cancelled';
    ...
    if (isTeachingShift) {
      ...
      if (zoomConfigured) {
        const { host, error: hostError } = await findAvailableHost(shiftStart, shiftEnd, shiftId);
        if (!host && hostError?.code === 'NO_AVAILABLE_HOST') {
          throw new functions.https.HttpsError(
            'failed-precondition',
            hostError.message || 'All Zoom hosts are at capacity for this time slot.',
            hostError
          );
        }
        selectedHost = host;
    ...
    // We NOW block if Zoom meeting creation fails to ensure shifts always have meetings.
    try {
      const zoomResult = await ensureZoomMeetingAndEmailTeacher({ shiftId, shiftData, selectedHost });

      if (zoomResult?.skipped === true) {
        // If it was skipped for a valid reason (e.g. non-teaching), that's fine
        if (['cancelled', 'non_teaching_shift', 'already_created'].includes(zoomResult.reason)) {
          return { success: true, results };
        }

        // Otherwise, skipping Zoom for a teaching shift is an error
        throw new functions.https.HttpsError(
          'aborted',
          `Zoom meeting creation skipped: ${zoomResult.reason || 'unknown reason'}. Shift created but Zoom meeting missing.`
        );
      }
    } catch (zoomError) {
      if (zoomError instanceof functions.https.HttpsError) {
        throw zoomError;
      }

      console.error(`[Zoom] Failed to ensure Zoom meeting for shift ${shiftId}:`, zoomError);

      // Throw error to trigger frontend rollback
      throw new functions.https.HttpsError(
        'internal',
        `Failed to create Zoom meeting: ${zoomError.message || String(zoomError)}. The shift will be removed.`
      );
    }
    return { success: true, results };
  } catch (error) {
...
const onShiftDeleted = onDocumentDeleted('teaching_shifts/{shiftId}', async (event) => {
  try {
    ...
    // Delete the Zoom meeting if one exists (smart cleanup)
    const zoomMeetingId = shiftData.zoom_meeting_id;
    if (zoomMeetingId) {
      try {
        // Check if other active/scheduled shifts are using this meeting
        const otherShifts = await getShiftsWithSameMeeting(zoomMeetingId);
        // Exclude the currently deleted shift (just in case query is fast)
        const activeOthers = otherShifts.filter(s => s.id !== shiftId);

        if (activeOthers.length > 0) {
          console.log(`[Zoom] Meeting ${zoomMeetingId} is shared by ${activeOthers.length} other shifts. Removing breakout room for ${shiftId}.`);

          // Re-build breakout rooms for the remaining shifts
          const breakoutRooms = await Promise.all(activeOthers.map(s => buildBreakoutRoom(s)));

          // Update the existing meeting with the new (smaller) list of rooms
          await updateZoomMeeting(zoomMeetingId, {
            breakoutRooms: breakoutRooms
          });

          console.log(`✅ Zoom meeting ${zoomMeetingId} patched (room for ${shiftId} removed)`);
        } else {
          // No other shifts using this meeting - safe to delete
          console.log(`[Zoom] No other shifts use meeting ${zoomMeetingId}. Deleting entire meeting.`);
          await deleteZoomMeeting(zoomMeetingId);
          console.log(`✅ Zoom meeting ${zoomMeetingId} deleted for shift ${shiftId}`);
        }
      } catch (zoomError) {
        // Log but don't fail the whole operation - meeting may already be deleted/inaccessible
        console.error(`⚠️ Failed to handle Zoom meeting cleanup for ${zoomMeetingId}:`, zoomError.message);
      }
    }
