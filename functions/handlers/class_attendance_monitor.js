// RETIRED — kept for history only, do not export from index.js.
// This is the RealtimeKit-era live-absence monitor (`monitorActiveClassAttendance`).
// It was deployed 2026-06-06 from a working tree that was never committed; the
// source here was recovered from the gcf-v2-sources deployment archive on
// 2026-07-13, when the deployed function was deleted. Its replacement is
// `detectClassAttendanceNoShows` in handlers/attendance.js, which covers Zoom
// classes and writes the same `class_attendance_alerts` collection.
...
// Automatic no-show detection only runs during the first N minutes of a class.
// After that, no automatic alert — a no-show only surfaces if someone manually
// reports it (reportNoShow).
const AUTO_ALERT_MAX_MINUTES = 10;
...
const evaluateClassAttendance = ({
  ...
  maxMinutes = AUTO_ALERT_MAX_MINUTES,
}) => {
  if (minutesSinceStart < graceMinutes) {
    return { problem: false, missing: null, withinGrace: true, afterWindow: false };
  }

  // Past the detection window: stop auto-flagging (manual reports still work).
  if (minutesSinceStart > maxMinutes) {
    return { problem: false, missing: null, withinGrace: false, afterWindow: true };
  }
  ...
  async () => {
    ...
    for (const doc of snap.docs) {
      ...
      try {
        ...
        // Only teaching classes get attendance monitoring. Leadership/admin
        // shifts (stored as `shift_category`) must never trigger alerts.
        const category = (
        ...
        // Automatic detection only during the first AUTO_ALERT_MAX_MINUTES minutes.
        // Past that, leave it to manual reports; close any open alert for hygiene.
        if (minutesSinceStart > AUTO_ALERT_MAX_MINUTES) {
          const openSnap = await alertRef.get();
          if (openSnap.exists && openSnap.data().closed !== true) {
            await alertRef.set(
              {
