...
const NO_SHOW_SCAN_LOOKBACK_MINUTES = 180;
...
const normalizeShiftRecord = (doc) => {
  ...
  const teacherName = `${data.teacher_name || data.teacherName || ''}`.trim();
  const studentNamesRaw = data.student_names || data.studentNames || [];
  const studentNames = Array.isArray(studentNamesRaw)
    ? studentNamesRaw.map((item) => `${item || ''}`.trim()).filter(Boolean)
    : [];
  ...
  const videoProvider = `${data.video_provider || data.videoProvider || ''}`.trim().toLowerCase();
  const className = `${data.custom_name || data.customName || data.auto_generated_name || data.autoGeneratedName || ''}`.trim() ||
    [teacherName, subjectName, studentNames.join(', ')].filter(Boolean).join(' - ') ||
    subjectName;
...
const missingStateForShift = ({
  ...
  const metricFor = (userId) =>
    metricsForShift?.get(userId) ||
    computeParticipantPresenceMetrics({
      sessionData: null,
      shiftStart: shift.shiftStart,
      shiftEnd: shift.shiftEnd,
  ...
  const teacherExpected = Boolean(shift.teacherId);
  const studentsExpected = Array.isArray(shift.studentIds) && shift.studentIds.length > 0;
  const teacherMissing = teacherExpected
    ? !metricFor(shift.teacherId).hadAnyPresence
    : false;
  const studentsMissing = studentsExpected
    ? !shift.studentIds.some((studentId) => metricFor(studentId).hadAnyPresence)
    : false;

  if (teacherMissing && studentsMissing) return 'both';
  if (teacherMissing) return 'teacher';
  if (studentsMissing) return 'students';
  return null;
};

const buildClassAttendanceAlertData = ({
  shift,
  missing,
  now = new Date(),
  existing = null,
}) => {
  const minutesSinceStart = shift.shiftStart
    ? Math.max(0, Math.floor((now.getTime() - shift.shiftStart.getTime()) / 60000))
    : null;
  const existingStatus = `${existing?.status || ''}`.trim().toLowerCase();
  const reviewed = existingStatus === 'reviewed' || existing?.reviewed_at || existing?.reviewedAt;
  const firstDetectedAt = toDate(existing?.first_detected_at || existing?.firstDetectedAt) || now;

  return {
    shift_id: shift.id,
    class_name: shift.className || shift.subjectName || 'Class',
    teacher_id: shift.teacherId || '',
    teacher_name: shift.teacherName || '',
    student_ids: shift.studentIds || [],
    student_names: shift.studentNames || [],
    missing,
    provider: shift.videoProvider || '',
    shift_start: admin.firestore.Timestamp.fromDate(shift.shiftStart),
    shift_end: admin.firestore.Timestamp.fromDate(shift.shiftEnd),
    first_detected_at: admin.firestore.Timestamp.fromDate(firstDetectedAt),
    last_detected_at: admin.firestore.Timestamp.fromDate(now),
    minutes_since_start: minutesSinceStart,
    source: 'scheduled_presence_detector',
...
const detectClassAttendanceNoShowsForWindow = async ({
  ...
  lookbackMinutes = NO_SHOW_SCAN_LOOKBACK_MINUTES,
} = {}) => {
  const scanStart = new Date(now.getTime() - lookbackMinutes * 60 * 1000);
  const detectBefore = new Date(now.getTime() - graceMinutes * 60 * 1000);
  if (detectBefore <= scanStart) {
    return { scannedShifts: 0, alertCount: 0, skipped: 0 };
  }

  const shifts = (await loadShiftsForPeriod({
    periodStart: scanStart,
    periodEnd: detectBefore,
  })).filter((shift) => shift.status !== 'cancelled');
  if (shifts.length === 0) {
    return { scannedShifts: 0, alertCount: 0, skipped: 0 };
  }

  const sessionsByShift = await loadSessionsByShift({
  ...
  for (const shift of shifts) {
    const missing = missingStateForShift({
      shift,
      metricsForShift: participantMetricsByShift.get(shift.id),
      now,
    });
    if (!missing) {
      skipped += 1;
    ...
    const alertRef = db
      .collection(CLASS_ATTENDANCE_ALERT_COLLECTION)
      .doc(shift.id);
    const existingDoc = await alertRef.get();
    const existing = existingDoc.exists ? existingDoc.data() || {} : null;
    const data = buildClassAttendanceAlertData({
      shift,
      missing,
      now,
      existing,
...
const detectClassAttendanceNoShows = onSchedule(
  { schedule: 'every 5 minutes', timeZone: 'Etc/UTC' },
  async () => {
    const result = await detectClassAttendanceNoShowsForWindow();
    console.log('[attendance] Class no-show detection complete:', result);
  },
);
