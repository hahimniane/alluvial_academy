...
import '../../../core/models/zoom_host.dart';
import '../../../core/services/shift_service.dart';
import '../../../core/services/subject_service.dart';
import '../../../core/services/zoom_host_service.dart';
...
class _CreateShiftDialogState extends State<CreateShiftDialog> {
  ...
  NoAvailableHostError? _extractNoAvailableHostError(Object error) {
    if (error is FirebaseFunctionsException) {
      final details = error.details;
      if (details is Map) {
        final mapped = details.map((key, value) => MapEntry('$key', value));
        return ZoomHostService.parseNoAvailableHostError(
          mapped.cast<String, dynamic>(),
        );
      }
    }
    return null;
  ...
  Future<void> _showNoAvailableHostDialog(NoAvailableHostError error) async {
    if (!mounted) return;

    await showDialog<void>(
      context: context,
      builder: (context) {
        final alternatives = error.alternatives;
        return AlertDialog(
          title: const Text('No Zoom host available'),
          content: SizedBox(
            width: 520,
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (error.suggestion != null &&
                      error.suggestion!.trim().isNotEmpty)
                    Padding(
                  ...
                  if (alternatives.isNotEmpty) ...[
                    ...
                    ...alternatives.map((slot) {
                      ...
                      final localEnd = TimezoneUtils.convertToTimezone(
                        slot.end.toUtc(),
                        _selectedTimezone,
                      );
                      final label =
                          '${DateFormat('EEE, MMM d').format(localStart)} • ${DateFormat('h:mm a').format(localStart)} - ${DateFormat('h:mm a').format(localEnd)} (${TimezoneUtils.getTimezoneAbbreviation(_selectedTimezone)})';
                      ...
                      return Padding(
                        ...
                        child: OutlinedButton(
                          onPressed: () {
                            ...
                            setState(() {
                              _shiftDate = DateTime(localStart.year,
                                  localStart.month, localStart.day);
                              _startTime = TimeOfDay(
                                  hour: localStart.hour,
                                  minute: localStart.minute);
                              _endTime = TimeOfDay(
                                  hour: localEnd.hour, minute: localEnd.minute);
                            });

                            ScaffoldMessenger.of(this.context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                    'Time updated to an available slot. Review and save again.'),
                              ),
                            );
                          },
                          child: Align(
  ...
  void _applyInitialValues() async {
    ...
    if (widget.initialStudentEmail != null &&
      ...
      try {
        ...
        AppLogger.debug(
            'CreateShiftDialog: Set initial student: ${student.email}');
      } catch (e) {
  ...
  Widget _buildConversionPreview() {
    ...
    final adminStartConverted =
        TimezoneUtils.convertToTimezone(utcStart, _adminTimezone);
    final adminEndConverted =
        TimezoneUtils.convertToTimezone(utcEnd, _adminTimezone);
  ...
  Future<void> _saveShift() async {
    ...
    try {
      ...
      if (mounted) {
        final parsed = _extractNoAvailableHostError(e);
        if (parsed != null) {
          await _showNoAvailableHostDialog(parsed);
        } else if (e is FirebaseFunctionsException) {
          // Handle Zoom-specific failures from shifts.js
          final message = e.message ?? e.toString();
          final isZoomError = message.toLowerCase().contains('zoom');

          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(isZoomError
                  ? 'Zoom Error: $message'
