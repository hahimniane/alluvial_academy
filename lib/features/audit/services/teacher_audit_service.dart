...
class TeacherAuditService {
  ...
  static Future<Map<String, dynamic>> _computeSingleTeacherAuditOptimized({
    ...
    try {
      ...
      final hoursForPayment = shiftMetrics.hoursWorkedBySubject.isNotEmpty
          ...
          : shiftMetrics
              .hoursBySubject; // Fallback to scheduled hours if worked hours not available
  ...
  static ShiftMetrics _processShifts(
    ...
    return ShiftMetrics(
      ...
      hoursWorkedBySubject:
          hoursWorkedBySubject, // Use scheduled hours as worked hours for now
  ...
  static bool _responsesLookLikeDailyClassReport(
    ...
    try {
      ...
      if (submittedAt != null) {
        ...
        if (submitted.isAfter(toleranceStart) &&
          ...
          return _verifyFormByClassDay(
              responses, startDate, endDate, yearMonth);
        }
      }
  ...
  static PaymentSummary _calculatePaymentFromTimesheets(
    ...
    final shiftDataMap =
        <String, Map<String, dynamic>>{}; // Store shift data for fallback
    ...
    for (final shift in shifts) {
      ...
      final hasFormBySuffix = shiftId.length >= 8 &&
          shiftsWithForms.contains(shiftId.substring(shiftId.length - 8));
      if (hasFormByExact || hasFormBySuffix) {
        ...
        if (kDebugMode) {
          if (status != 'fullyCompleted' &&
              ...
              status != 'partiallyCompleted') {
            AppLogger.debug(
                'Shift $shiftId has form but status="$status" - including for payment (form = proof of work)');
          }
        }
      }
    }
    ...
    if (kDebugMode) {
      AppLogger.debug(
          'Built eligibleShiftIds: ${eligibleShiftIds.length} shifts');
      AppLogger.debug(
          'Built shiftSubjectMap: ${shiftSubjectMap.length} entries');
      if (eligibleShiftIds.isNotEmpty) {
        AppLogger.debug(
            'Sample eligible shift IDs: ${eligibleShiftIds.take(5).join(", ")}');
      }
    }
    ...
    final formLookupMap =
        <String, Map<String, dynamic>>{}; // full shiftId -> form data
    for (final form in detailedForms) {
      ...
      for (final entry in shiftDataMap.entries) {
        ...
        if (formShiftId.length >= 8 &&
            shiftId.length >= 8 &&
            shiftId.substring(shiftId.length - 8) ==
                (formShiftId.length > 8
                    ? formShiftId.substring(formShiftId.length - 8)
                    : formShiftId)) {
          formLookupMap[shiftId] = form;
          break;
        }
    ...
    for (final shiftId in eligibleShiftIds) {
      ...
      for (final ts in timesheets) {
        ...
        final tsMatches = tsShiftId == shiftId ||
            (shiftId.length >= 8 &&
                tsShiftId != null &&
                tsShiftId.isNotEmpty &&
                (tsShiftId.length >= 8
                        ? tsShiftId.substring(tsShiftId.length - 8)
                        : tsShiftId) ==
                    shiftId.substring(shiftId.length - 8));
        if (tsMatches) {
          ...
          final clockIn = (tsDataMap['clock_in_timestamp'] as Timestamp? ??
                  tsDataMap['clock_in'] as Timestamp?)
              ?.toDate();
          final clockOut =
              (tsDataMap['clock_out_timestamp'] as Timestamp?)?.toDate();
          ...
          if (payment == 0 && hours > 0) {
            ...
            if (hourlyRate > 0) {
              ...
              if (kDebugMode) {
                AppLogger.debug(
                    '[PRIORITY 1] Calculated payment from timesheet for shift $shiftId: ${hours.toStringAsFixed(2)}h × \$${hourlyRate.toStringAsFixed(2)} = \$${payment.toStringAsFixed(2)}');
              }
            }
          }
      ...
      if (payment == 0 && formLookupMap.containsKey(shiftId)) {
        ...
        if (kDebugMode) {
          AppLogger.debug(
              '[PRIORITY 2] Attempting fallback payment for shift $shiftId: formDuration=${formDuration.toStringAsFixed(2)}h');
        }
        ...
        if (formDuration > 0) {
          ...
          if (kDebugMode) {
            AppLogger.debug(
                '[PRIORITY 2] Shift $shiftId: hourlyRate=\$${hourlyRate.toStringAsFixed(2)}');
          }
          ...
          if (hourlyRate > 0) {
            ...
            if (kDebugMode) {
              AppLogger.debug(
                  '[PRIORITY 2] ✅ Calculated payment from form duration for shift $shiftId: ${formDuration.toStringAsFixed(2)}h × \$${hourlyRate.toStringAsFixed(2)} = \$${payment.toStringAsFixed(2)}');
            }
          } else {
            if (kDebugMode) {
              AppLogger.debug(
                  '[PRIORITY 2] ⚠️ Shift $shiftId has form with duration ${formDuration.toStringAsFixed(2)}h but no hourly rate found - cannot calculate payment');
            }
          }
        } else {
          if (kDebugMode) {
            AppLogger.debug(
                '[PRIORITY 2] ⚠️ Shift $shiftId has form but formDuration is 0 or invalid - cannot calculate payment');
          }
        }
      } else if (payment == 0 && !formLookupMap.containsKey(shiftId)) {
        if (kDebugMode) {
          AppLogger.debug(
              '[PRIORITY 2] ⚠️ Shift $shiftId has no timesheet payment AND no form found in formLookupMap - will result in \$0 payment');
        }
      }
    ...
    if (adjustmentsToApply.isNotEmpty) {
      if (kDebugMode) {
        AppLogger.debug(
            'Applying ${adjustmentsToApply.length} admin adjustments...');
      }
      for (final entry in adjustmentsToApply.entries) {
        ...
        if (eligibleShiftIds.contains(shiftId)) {
          ...
          shiftPayments[shiftId] =
              adjustedAmount; // Use adjusted amount instead of calculated
          if (kDebugMode) {
            AppLogger.debug(
                '  Shift $shiftId: \$${oldPayment.toStringAsFixed(2)} → \$${adjustedAmount.toStringAsFixed(2)} (adjusted)');
          }
        }
      }
    }
    ...
    if (kDebugMode) {
      AppLogger.debug(
          'Grouping ${shiftPayments.length} shift payments by subject...');
    }
    ...
    for (final entry in shiftPayments.entries) {
      ...
      if (kDebugMode && !shiftSubjectMap.containsKey(shiftId)) {
        AppLogger.debug(
            '  ⚠️ Shift $shiftId not found in shiftSubjectMap, using "Other"');
      }

      subjectPayments[subject] = (subjectPayments[subject] ?? 0) + payment;
      subjectHours[subject] =
          (subjectHours[subject] ?? 0) + (shiftHours[shiftId] ?? 0);

      if (kDebugMode) {
        AppLogger.debug(
            '  Shift $shiftId: \$${payment.toStringAsFixed(2)} → Subject "$subject" (new total: \$${subjectPayments[subject]!.toStringAsFixed(2)})');
      }
    }

    if (kDebugMode) {
      AppLogger.debug(
          'Subject payments after grouping: ${subjectPayments.entries.map((e) => '${e.key}: \$${e.value.toStringAsFixed(2)}').join(", ")}');
    }
    ...
    if (kDebugMode) {
      AppLogger.debug(
          'Building SubjectPayment objects from ${subjectPayments.length} subjects...');
      AppLogger.debug(
          'totalGross BEFORE loop: \$${totalGross.toStringAsFixed(2)}');
    }
    ...
    for (final entry in subjectPayments.entries) {
      ...
      if (kDebugMode) {
        AppLogger.debug(
            '  Subject "$subject": \$${totalPayment.toStringAsFixed(2)} → totalGross now: \$${totalGross.toStringAsFixed(2)}');
      }
    }

    if (kDebugMode) {
      AppLogger.debug(
          'totalGross AFTER loop: \$${totalGross.toStringAsFixed(2)}');
    }
    ...
    if (kDebugMode) {
      ...
      AppLogger.debug(
          'Eligible shifts (ANY shift with form - form = proof of work): ${eligibleShiftIds.length}');
      AppLogger.debug(
          'Shifts with payment data in shiftPayments map: ${shiftPayments.length}');
      AppLogger.debug('Subject payments map size: ${subjectPayments.length}');
      AppLogger.debug(
          'Payments by subject: ${subjectPayments.entries.map((e) => '${e.key}: \$${e.value.toStringAsFixed(2)}').join(", ")}');
      AppLogger.debug(
          'Existing adjustments: ${existingAdjustments?.length ?? 0}');
      ...
      if (shiftPayments.isNotEmpty) {
        ...
        for (var entry in shiftPayments.entries.take(10)) {
          ...
          AppLogger.debug(
              '  Shift $shiftId ($subject): \$${payment.toStringAsFixed(2)}');
        }
      ...
      AppLogger.debug(
          'Total Gross (before penalties): \$${totalGross.toStringAsFixed(2)}');
      AppLogger.debug(
          'Total Penalties: \$${totalPenalties.toStringAsFixed(2)}');
      AppLogger.debug(
          'Total Net (after penalties): \$${(totalGross - totalPenalties).toStringAsFixed(2)}');

      // Check if totals don't match individual payments
      final sumOfIndividualPayments =
          shiftPayments.values.fold(0.0, (sum, payment) => sum + payment);
      if (sumOfIndividualPayments != totalGross) {
        AppLogger.debug(
            '⚠️ MISMATCH: Sum of individual payments (\$${sumOfIndividualPayments.toStringAsFixed(2)}) != Total Gross (\$${totalGross.toStringAsFixed(2)})');
      }

      if (shiftsWithForms.isNotEmpty && shiftPayments.isEmpty) {
        AppLogger.debug(
            '⚠️ CRITICAL: Shifts have forms but no payment data found in timesheets!');
      }
    }
  ...
  static Future<TeacherAuditFull?> getAudit({
    ...
    try {
      ...
      final doc =
          await _firestore.collection(_auditCollection).doc(docId).get();
  ...
  static const Set<AuditStatus> teacherVisibleStatuses = {
  ...
  static bool isTeacherVisibleStatus(AuditStatus status) =>
      teacherVisibleStatuses.contains(status);

  static bool hasTeacherResponded(TeacherAuditFull? audit) {
    if (audit == null) return false;
    return audit.reviewChain?.teacherReview != null ||
        audit.reviewChain?.teacherDispute != null ||
        audit.teacherAcknowledgedAt != null;
  }

  static bool didTeacherApprove(TeacherAuditFull? audit) {
    if (audit == null) return false;
    final teacherReview = audit.reviewChain?.teacherReview;
    if (teacherReview != null) {
      return teacherReview.status == 'approved';
    }
    return audit.teacherAcknowledgedAt != null &&
        audit.reviewChain?.teacherDispute == null;
  }

  static bool didTeacherDisapprove(TeacherAuditFull? audit) {
    if (audit == null) return false;
    final teacherReview = audit.reviewChain?.teacherReview;
    if (teacherReview != null) {
      return teacherReview.status == 'rejected';
    }
    return audit.reviewChain?.teacherDispute != null;
  }

  static DateTime? teacherRespondedAt(TeacherAuditFull? audit) {
    if (audit == null) return null;
    return audit.reviewChain?.teacherReview?.reviewedAt ??
        audit.reviewChain?.teacherDispute?.disputedAt ??
        audit.teacherAcknowledgedAt;
  }
  ...
  static Future<bool> submitTeacherReview({
    required String auditId,
    required bool approved,
    String notes = '',
    String disputedField = 'other',
    ...
    if (!approved && trimmedNotes.isEmpty) {
      return false;
    }
    ...
    try {
      ...
      final audit = TeacherAuditFull.fromFirestore(doc);
      if (audit.oderId != user.uid) {
        AppLogger.warning(
          'submitTeacherReview: user ${user.uid} is not allowed to review audit $auditId',
        );
        return false;
      }
      ...
      final updates = <String, dynamic>{
        'reviewChain.teacherReview': review.toMap(),
        'lastUpdated': FieldValue.serverTimestamp(),
      };

      if (approved) {
        updates['teacherAcknowledgedAt'] = FieldValue.serverTimestamp();
        updates['reviewChain.teacherDispute'] = FieldValue.delete();
      } else {
        final dispute = TeacherDispute(
          teacherId: user.uid,
          disputedAt: DateTime.now(),
          field: disputedField.trim().isEmpty ? 'other' : disputedField.trim(),
          reason: trimmedNotes,
          suggestedValue: suggestedValue,
          status: 'pending',
          adminResponse: '',
        );
        updates['reviewChain.teacherDispute'] = dispute.toMap();
        updates['teacherAcknowledgedAt'] = FieldValue.delete();
        updates['status'] = AuditStatus.disputed.name;
      }

      await docRef.update(updates);

      await _notifyLeadersOfTeacherDecision(
        auditId: audit.id,
  ...
  static Future<void> _notifyLeadersOfTeacherDecision({
    required String auditId,
    required String yearMonth,
    required bool approved,
    ...
    try {
      final callable = FirebaseFunctions.instanceFor(region: 'us-central1')
          .httpsCallable('sendTeacherAuditDecisionNotification');
      ...
      AppLogger.error(
        'sendTeacherAuditDecisionNotification callable failed: $e',
        error: e,
        stackTrace: st,
      );
    }
  ...
  static Future<bool> submitDispute({
    ...
    try {
      ...
      await docRef.update({
        ...
        'teacherAcknowledgedAt': FieldValue.delete(),
  ...
  static Future<bool> linkFormToShift({
    ...
    try {
      ...
      final teacherId =
          formData['userId'] as String? ?? formData['submitted_by'] as String?;
