...
class _ManageGuardiansDialogState extends State<ManageGuardiansDialog> {
  ...
  /// The Cloud Function rejects removing a minor's last guardian with a
  /// `failed-precondition` error tagged `last_guardian_minor` in its details.
  ...
  Future<void> _confirmUnlink(GuardianLink guardian) async {
    ...
    try {
      ...
      setState(() {
        _guardians =
            _guardians.where((g) => g.id != guardian.id).toList();
        _unlinkingId = null;
      });
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _isMinorBlock(e) ? l.unlinkGuardianMinorBlocked : l.unlinkGuardianError,
          ),
          backgroundColor: const Color(0xffDC2626),
