...
class ChatPermissionService {
  ...
  Future<bool> canMessage(String userId1, String userId2) async {
    ...
    try {
      ...
      final user1Role =
          (user1Data['user_type'] as String?)?.toLowerCase() ?? '';
      final user2Role =
          (user2Data['user_type'] as String?)?.toLowerCase() ?? '';
  ...
  Future<List<ChatUser>> getEligibleContacts(String userId) async {
    try {
      ...
      AppLogger.error(
          'ChatPermissionService: Error getting eligible contacts: $e');
      return [];
    }
  }

  /// Get eligible contacts grouped by relationship type
  Future<Map<String, List<ChatUser>>> getEligibleContactsGrouped(
      String userId) async {
    try {
      final userDoc = await _firestore.collection('users').doc(userId).get();
      if (!userDoc.exists) return {};
      ...
      // Always add admins
      final admins = await _getAdminUsers();
      groupedContacts['Administrators'] =
          admins.where((a) => a.id != userId).toList();
      ...
      AppLogger.error(
          'ChatPermissionService: Error getting grouped contacts: $e');
      return {};
    }
  }

  /// Check if teacher has an active teaching relationship with student
  Future<bool> hasActiveTeachingRelationship(
      String teacherId, String studentId) async {
    try {
      final cutoffDate =
          DateTime.now().subtract(Duration(days: _shiftLookbackDays));
      ...
      AppLogger.error(
          'ChatPermissionService: Error checking teaching relationship: $e');
      return false;
    }
  }

  /// Check if teacher can message a parent (via teaching their children)
  Future<bool> canTeacherMessageParent(
      String teacherId, String parentId) async {
    try {
      ...
      AppLogger.error(
          'ChatPermissionService: Error checking teacher-parent permission: $e');
      return false;
    }
  }

  /// Get the relationship type between two users
  Future<ChatRelationshipType> getRelationshipType(
    ...
    try {
      ...
      final user1Role =
          (user1Doc.data()!['user_type'] as String?)?.toLowerCase() ?? '';
      final user2Role =
          (user2Doc.data()!['user_type'] as String?)?.toLowerCase() ?? '';

      // Check for admin relationship
      if (user1Role == 'admin' ||
          user1Role == 'super_admin' ||
          user2Role == 'admin' ||
          user2Role == 'super_admin') {
      ...
      AppLogger.error(
          'ChatPermissionService: Error getting relationship type: $e');
      return ChatRelationshipType.none;
    }
  }
  ...
  Future<String?> getRelationshipContext(String userId1, String userId2) async {
    ...
    if (relationship == ChatRelationshipType.none &&
      ...
      final otherDoc = await _firestore.collection('users').doc(userId2).get();
  ...
  Future<bool> _otherPartyHasMessagedInDirectChat(
    ...
    try {
      ...
      final last = chatDoc.data()?['last_message'] as Map<String, dynamic>?;
  ...
  Future<List<ChatUser>> _getAdminUsers() async {
    final snapshots = await Future.wait([
      _firestore.collection('users').where('user_type', whereIn: [
        'admin',
        'super_admin',
        'ceo',
        'founder',
    ...
    final leadersById = <String, ChatUser>{};
    for (final snapshot in snapshots) {
      for (final doc in snapshot.docs) {
        final data = doc.data();
        if (_isPrivilegedAdmin(data)) {
          leadersById[doc.id] = _docToChatUser(doc);
        }
      }
    }
    return leadersById.values.toList();
  }
  ...
  Future<List<ChatUser>> _getTeacherStudentParents(String teacherId) async {
    ...
    for (final student in students) {
      final studentDoc =
          await _firestore.collection('users').doc(student.id).get();
      if (studentDoc.exists) {
        final guardianIds =
            List<String>.from(studentDoc.data()!['guardian_ids'] ?? []);
