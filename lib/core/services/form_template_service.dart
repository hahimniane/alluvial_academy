...
class FormTemplateService {
  ...
  static Future<List<FormTemplate>> getAllTemplates({bool forceRefresh = false}) async {
    try {
      ...
      try {
        ...
        // #region agent log
        final logData = templates.map((t) => {'id': t.id, 'name': t.name, 'version': t.version, 'isActive': t.isActive}).toList();
  ...
  static Future<FormTemplate?> getActiveDailyTemplate({bool forceRefresh = true}) async {
    try {
      // #region agent log
      await _writeDebugLog('form_template_service.dart:1032', 'getActiveDailyTemplate called', {'forceRefresh': forceRefresh}, 'A');
      // #endregion
      ...
      // #region agent log
      await _writeDebugLog('form_template_service.dart:1045', 'Config doc checked', {
        'exists': configDoc.exists,
        'dailyTemplateId': configDoc.data()?['dailyTemplateId'],
      }, 'A');
      // #endregion
      ...
      if (configDoc.exists) {
        ...
        if (dailyTemplateId != null) {
          // #region agent log
          await _writeDebugLog('form_template_service.dart:1049', 'Fetching template from config ID', {'dailyTemplateId': dailyTemplateId}, 'A');
          // #endregion
          final template = await getTemplate(dailyTemplateId, forceRefresh: forceRefresh);
          // #region agent log
          await _writeDebugLog('form_template_service.dart:1051', 'Template from config', {
      ...
      // #region agent log
      final allTemplatesData = allTemplates.map((t) => {'id': t.id, 'name': t.name, 'version': t.version, 'updatedAt': t.updatedAt.toIso8601String(), 'isActive': t.isActive}).toList();
      await _writeDebugLog('form_template_service.dart:1057', 'All perSession templates found', {
      ...
      if (allTemplates.isNotEmpty) {
        ...
        // #region agent log
        final dedupedTemplatesData = templates.map((t) => {'id': t.id, 'name': t.name, 'version': t.version, 'updatedAt': t.updatedAt.toIso8601String()}).toList();
        await _writeDebugLog('form_template_service.dart:1093', 'After deduplication', {
          'count': templates.length,
          'templates': dedupedTemplatesData,
        }, 'B');
        ...
        // #region agent log
        final sortedTemplatesData = templates.map((t) => {'id': t.id, 'name': t.name, 'version': t.version, 'updatedAt': t.updatedAt.toIso8601String()}).toList();
        await _writeDebugLog('form_template_service.dart:1101', 'After sorting by version', {'templates': sortedTemplatesData}, 'B');
        // #endregion
        ...
        // #region agent log
        await _writeDebugLog('form_template_service.dart:1111', 'Selected daily template', {
          'id': dailyReport.id,
          'name': dailyReport.name,
          'version': dailyReport.version,
          'updatedAt': dailyReport.updatedAt.toIso8601String(),
        }, 'B');
