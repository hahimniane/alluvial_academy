...
| AI Tutor | `/teacher/tutor/` | Conditional `ai_tutor_enabled` entry, guarded disabled state, Flutter-compatible mode/voice/background preferences, real token request, LiveKit room lifecycle, microphone/text/transcripts, synchronized version-2 whiteboard topics, and AI-requested GPS clock-in/reschedule action/result channels | Dev LiveKit signaling currently returns HTTP 404/CORS failure, preventing real agent/channel acceptance until the shared endpoint is repaired |
| Savings Circles | `/teacher/circles/` | Conditional `tontine_enabled` navigation/guard, invitations, open teacher-circle joining, creator/member dashboards, circle/member creation, activation, cycle/payment board, receipt submission, head confirmation/rejection, and payout completion using Flutter collections and Storage paths | Multi-member invited-user acceptance and generated-cycle Function acceptance |
